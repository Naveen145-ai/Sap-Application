...
exports.submitSAPForm = async (req, res) => {
  try {
    ...
    const proof = req.file?.path; // Cloudinary URL
...
exports.submitIndividualEvent = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length) {
      console.log('Received file fields:', req.files.map(f => ({ field: f.fieldname, name: f.originalname, size: f.size })));
    }

    // ✅ Handle file uploads mapped by category
    const files = Array.isArray(req.files) ? req.files : [];
    const proofUrlsByCategory = {}; // ✅ Store proof URLs by category
    
    files.forEach(f => {
      // Parse fieldname like: proofs[paperPresentation][insidePresented]
      const match = f.fieldname.match(/^proofs\[([^\]]+)\]\[([^\]]+)\]$/);
      if (match) {
        const [, eventKey, categoryKey] = match;
        if (!proofUrlsByCategory[categoryKey]) {
          proofUrlsByCategory[categoryKey] = [];
        }
        proofUrlsByCategory[categoryKey].push(f.path);
      }
    });
    ...
    const newEvent = {
      ...
      values: {
        ...parsedEventData,
        proofUrlsByCategory // ✅ Store category-specific proof URLs
      },
      proofUrls: Object.values(proofUrlsByCategory).flat(), // ✅ Also keep flat array for backward compatibility
      mentorMarks: {},
      status: 'pending'
    };
    ...
    // Build enriched values including proofUrlsByCategory for rendering
    const enrichedValues = {
      ...parsedEventData,
      proofUrlsByCategory, // ✅ keep category mapping for mentor UI
    };
    const flatProofs = Object.values(proofUrlsByCategory).flat();
    ...
    if (existingSubmission) {
      ...
      if (eventIndex !== -1) {
        existingSubmission.events[eventIndex] = {
          ...
          proofUrls: flatProofs, // ✅ Keep flat list for backward compatibility
